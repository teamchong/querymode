...
2. **Agents need live data.** An agent making decisions on outdated training data is worse than no agent at all. Agents make API calls for live data — lots of them. That data needs to live at the edge, close to where agents run.
